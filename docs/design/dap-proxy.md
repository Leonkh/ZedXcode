...
1. **lldb-dap proxy flow:**
   ...
   - **Pidfile per sim** (`sim-<udid>.pid`): claimed after a successful pipeline run (the udid is known only post device-resolution), SIGTERMing a stale previous instance then, since a Rerun can race the old session's teardown; removed on teardown. See §3.3.
...
- *stdin EOF* → cleanup, exit 0. *SIGTERM/SIGINT* (tokio::signal) → same cleanup path. Cleanup = kill children (`kill_on_drop` + explicit pgid kills), drop tailers, remove pidfile.
- *Pidfile*: claimed **after a successful pipeline run** (post device-resolution — the udid is not known at launch-intercept): read `~/.zedxcode/run/sim-<udid>.pid`, SIGTERM a stale previous instance, write own pid; removed on teardown.
